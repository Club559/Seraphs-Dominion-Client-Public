...
public class GATracker implements AnalyticsTracker {
    ...
    private function _trackerFactory():GoogleAnalyticsAPI {
        ...
        var _adSense:AdSenseGlobals = new AdSenseGlobals(debug);
        var _dom:HTMLDOM = new HTMLDOM(debug);
        _dom.cacheProperties();
        var _env:Environment = new Environment("", "", "", debug, _dom);
        var _buffer:Buffer = new Buffer(config, debug, false);
        var _gifRequest:GIFRequest = new GIFRequest(config, debug, _buffer, _env);
        var _idleTimer:IdleTimer = new IdleTimer(config, debug, _display, _buffer);
        var _ecom:Ecommerce = new Ecommerce(_debug);
